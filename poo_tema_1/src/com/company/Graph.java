...
public class Graph {
    ...
    *Searches for the street with the given start and end and adds a traffic instance to the list
    ...
    /**
     * used method to check if the readGraph method works corectly
     */
    public void printGraph(){
        for(String keys : graph.keySet()){
            for(int i = 0; i < graph.get(keys).size(); i++) {
                System.out.print(keys + " ");
                graph.get(keys).get(i).printStreet();
            }
        }
    }
    ...
    /**
     *
     * @param x vehicle to calculate cost for each street
     * @return new maps with updated costs
     */
    public Graph calculateCostForVehicle(Vehicle x){

        Graph map = new Graph();
        for(String key : graph.keySet()){
            for(int i = 0; i < graph.get(key).size(); i++){
                int cost = x.calculateCost(graph.get(key).get(i));
                map.addStreet(key,graph.get(key).get(i).getDestination(),cost,graph.get(key).get(i).getSize());
            }
        }
    ...
    /**
     *
     * @param x vehicle that runs on the streets
     * @return updated map
     * if the cost is -1 then the size of the vehicle is bigger than the size of the street,so the street is deleted
     * from the map
     */
    public Graph getMapForVehicle(Vehicle x){
        Graph map = calculateCostForVehicle(x);
        for(String key : map.graph.keySet()){
            for(int i = 0; i < map.graph.get(key).size(); i++){
                if(map.graph.get(key).get(i).getCost() < 0)
                    map.graph.get(key).remove(i);
            }
        }
